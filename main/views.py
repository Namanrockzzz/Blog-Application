...
def index(request):
    # Queries are executed lazily i.e. not untill latest_articles variable is called by some function or so
    latest_articles = models.Article.objects.all().order_by('-createdAt')[:10]
    # - is added before name of the field to get decreasing order
